...
def add_impulse_noise_to_obs(obs, severity=1):
    """
    Add impulse (salt & pepper) noise to RGB observations.
    severity: int in [1, 5]
    """
    obs = copy.deepcopy(obs)

    for cam in ['rgb_static', 'rgb_gripper']:
        img = obs['rgb_obs'][cam]  # uint8, HWC

        # imagecorruptions expects uint8 HWC
        noisy = corrupt(
            img,
...
def apply_corruption_to_obs(obs, noise_type, severity):
    """
    Apply noise corruption to observations based on noise_type.
    If noise_type is "none", return directly without modification.
    """
    if noise_type == "none" or noise_type is None or severity == 0:
        return obs
        
    obs = copy.deepcopy(obs)
    for cam in ['rgb_static', 'rgb_gripper']:
        if cam in obs['rgb_obs']:
            img = obs['rgb_obs'][cam]  # uint8, HWC
            try:
                noisy = corrupt(img, corruption_name=noise_type, severity=severity)
                obs['rgb_obs'][cam] = noisy
            except KeyError:
                print(f"Warning: Noise type {noise_type} not found. Skipping.")
                return obs
    return obs
...
def evaluate_policy(model, env, eval_sr_path, eval_result_path, num_procs, procs_id, eval_dir, ep_len, num_sequences, task_name='test', enrich_lang=False, debug=False, cfg=None):
    ...
    # val_annotations = {key: val for key, val in val_annotations.items() if 'push' in key and 'right' in key}
    ...
    for initial_state, eval_sequence in eval_sequences:
        result = evaluate_sequence(env, model, task_oracle, initial_state, eval_sequence, val_annotations, debug, eval_dir, sequence_i, ep_len, cfg)
...
def evaluate_sequence(env, model, task_checker, initial_state, eval_sequence, val_annotations, debug, eval_dir, sequence_i, ep_len, cfg=None):
    ...
    for subtask_i, subtask in enumerate(eval_sequence):
        success = rollout_hi3(env, model, task_checker, subtask, val_annotations, debug, eval_dir, subtask_i, sequence_i, ep_len, cfg)
        if success:  # return 5!!!!!!!!!!!!!!!!!!!
...
def rollout(env, model, task_oracle, subtask, val_annotations, debug, eval_dir, subtask_i, sequence_i, ep_len, cfg=None):
    if debug:
        print(f"{subtask} ", end="")
        time.sleep(0.5)
    obs = env.get_obs()
...
def rollout_hi3(env, model, task_oracle, subtask, val_annotations, debug, eval_dir, subtask_i, sequence_i, ep_len, cfg=None):
    if debug:
        print(f"{subtask} ", end="")
        time.sleep(0.5)

    obs = env.get_obs()
    if cfg.noise_type != "none" and cfg.noise_severity > 0:
        obs = apply_corruption_to_obs(obs, cfg.noise_type, cfg.noise_severity)

    lang_annotation = val_annotations[subtask][0]
    model.reset()
    start_info = env.get_info()

    img_dict = {'static': [], 'gripper': []} if cfg.record_video else None
...
def main(cfg: GenerateConfig):
    with threadpool_limits(limits=3): 
        print(f"✅ Enforcing thread limit=3 for PID {os.getpid()}")
        ...
        env = make_env('data/task_ABC_D', observation_space, DEVICE)
