...
from prismatic.util.nn_utils import FusedMLPProjector, LinearProjector, MLPProjector, FusedFANProjector
...
class PrismaticVLM(VLM):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        proj_sd = model_state_dict["projector"]
        fixed_proj_sd = {}

        for k, v in proj_sd.items():
            new_k = k
            new_k = new_k.replace("fan_block.mlp.0", "fan_block.mlp_fc1")
            new_k = new_k.replace("fan_block.mlp.2", "fan_block.mlp_fc2")
            fixed_proj_sd[new_k] = v

        vlm.projector.load_state_dict(fixed_proj_sd)
