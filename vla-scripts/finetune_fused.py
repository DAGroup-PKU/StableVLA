...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if cfg.use_minivlm:
        ...
        elif 'fusedfan-projector' in cfg.vlm_path:
            vlm = load(cfg.vlm_path, hf_token=hf_token, load_for_training=True)
        elif 'llava-lvis4v-lrv+prism-qwen25-extra-dinosiglip-224px+0_5b+fan-projector+stage-finetune+x7' in cfg.vlm_path:
        # fan VLM
            vlm = load(cfg.vlm_path, hf_token=hf_token, load_for_training=True)
        else:
            vlm = load_vla(
        ...
        replace_map = [
            ...
            ("gamma", "scale_factor"),
            # ("fan_block.mlp.0", "fan_block.mlp_fc1"),
            # ("fan_block.mlp.2", "fan_block.mlp_fc2"),
            ("clean_body.0", "clean_fc1"),
            ("clean_body.2", "clean_fc2"),
            ]
    ...
    if cfg.use_lora:
        ...
        if cfg.resume:
            from pathlib import Path
            lora_adapter_path = Path(cfg.resum_vla_path) / "lora_adapter"
            if lora_adapter_path.exists():
                print(f"Loading LoRA adapter from: {lora_adapter_path}")
                from peft import set_peft_model_state_dict
                import safetensors.torch
                adapter_weights = safetensors.torch.load_file(lora_adapter_path / "adapter_model.safetensors")
                set_peft_model_state_dict(vla, adapter_weights)
    ...
    # Start training
    initial_step = cfg.resume_step if cfg.resume and cfg.resume_step else 0
    with tqdm.tqdm(total=cfg.max_steps, initial=initial_step, leave=False) as progress:
        vla.train()
        optimizer.zero_grad()
        for batch_idx, batch in enumerate(dataloader):
            ...
            # === Check FAN vs Clean gradients ===
            if batch_idx % 100 == 0 and distributed_state.is_main_process:
                clean_grad_norm = 0
                fan_grad_norm = 0
                clean_count = 0
                fan_count = 0
                ...
                for name, param in vla.named_parameters():
                    if param.grad is not None and 'projector' in name:
                        grad_norm = param.grad.norm().item()
                        if 'clean_fc' in name:
                            clean_grad_norm += grad_norm
                            clean_count += 1
                        elif 'fan_' in name or 'fan_block' in name:
                            fan_grad_norm += grad_norm
                            fan_count += 1

                if clean_count > 0 and fan_count > 0:
                    avg_clean = clean_grad_norm / clean_count
                    avg_fan = fan_grad_norm / fan_count
                    ratio = avg_fan / avg_clean if avg_clean > 0 else 0
                    print(f"[Step {batch_idx}] Grad - Clean: {avg_clean:.6f}, FAN: {avg_fan:.6f}, Ratio(FAN/Clean): {ratio:.4f}")
