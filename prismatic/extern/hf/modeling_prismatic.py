...
class PrismaticProjector(nn.Module):
    def __init__(
        ...
        if arch_specifier.endswith("fusedfan-projector"):
            self.projector_type = "fusedfan"
            from prismatic.util.nn_utils import EfficientChannelAttention
            initial_projection_dim = self.vision_dim * 4
            self.clean_fc1 = nn.Linear(self.vision_dim, initial_projection_dim, bias=True)
            self.clean_act1 = nn.GELU()
            self.clean_fc2 = nn.Linear(initial_projection_dim, self.llm_dim, bias=True)
            self.clean_act2 = nn.GELU()

            self.fan_in_proj = nn.Linear(self.vision_dim, self.llm_dim, bias=True)
            self.act = nn.GELU()
            self.fan_block = EfficientChannelAttention(self.llm_dim, num_heads=16, mlp_ratio=4.0)

            # Part 3: Shared Tail
            self.shared_tail = nn.Linear(self.llm_dim, self.llm_dim, bias=True)
            self.layer_norm = nn.LayerNorm(self.llm_dim)
            self.register_buffer('gate', torch.tensor([0.3]))
            self.clean_dropout = nn.Dropout(p=0.3)

        elif not self.use_fused_vision_backbone:
    ...
    def forward(self, img_patches: torch.Tensor) -> torch.Tensor:
        if self.projector_type == "fusedfan":
            feat_clean = self.clean_fc1(img_patches)
            feat_clean = self.clean_act1(feat_clean)
            feat_clean = self.clean_fc2(feat_clean)
            feat_clean = self.clean_act2(feat_clean)

            if self.training:
                feat_clean = self.clean_dropout(feat_clean)

            # 2. FAN Body
            feat_fan = self.fan_in_proj(img_patches)
            feat_fan = self.act(feat_fan)
            feat_robust = self.fan_block(feat_fan)

            # 3. Fusion + Shared Tail
            combined_feat = feat_clean + torch.tanh(self.gate) * feat_robust
            out = self.layer_norm(combined_feat)
            out = self.shared_tail(out)
            return out
        
        elif self.projector_type == "fan_ablation_softmax":
            # FAN Projector forward
            x = self.fc1(img_patches)
            x = self.act(x)

            residual = x
            x_clean = self.fan_block(x)
            x = self.layer_norm(residual + x_clean)

            x = self.fc2(x)
            return x
        
        elif self.projector_type == "fan_ablation_linear":
            # FAN Projector forward
            x = self.fc1(img_patches)
            x = self.act(x)
        ...
        # elif self.projector_type == "fanhybrid":
        #     # FANHybrid Projector forward
        #     # 1. FAN: Denoise in vision space first
        #     x = self.pre_fan_norm(img_patches)
        #     residual = x
        #     x = self.fan_block(x)
        #     x = self.post_fan_norm(residual + x)

        #     # 2. FusedMLP: Project "clean" vision features to LLM space (independent layers)
        #     x = self.fused_fc1(x)
        #     x = self.fused_act1(x)
        #     x = self.fused_fc2(x)
        #     x = self.fused_act2(x)
