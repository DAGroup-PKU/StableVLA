...
class GenerateConfig:
    ...
    corruption_type: str = 'impulse_noise' 
...
def setup_logging(cfg: GenerateConfig):
    ...
    # Add noise type and severity to filename if enabled
    if cfg.corruption_severity > 0:
        run_id += f"--{cfg.corruption_type}_sev{cfg.corruption_severity}"
...
# Add severity and corruption_name parameters
def prepare_observation(obs, resize_size, severity=0, corruption_name='impulse_noise'):
    """Prepare observation for policy input."""
    # Get preprocessed images (original uint8 images)
    img = get_libero_image(obs)
    wrist_img = get_libero_wrist_image(obs)

    # Apply image corruption if severity > 0
    if severity > 0:
        # imagecorruptions requires uint8 input, outputs uint8
        img = corrupt(img, severity=severity, corruption_name=corruption_name)
        wrist_img = corrupt(wrist_img, severity=severity, corruption_name=corruption_name)
    ...
    try:
        while t < max_steps + cfg.num_steps_wait:
            ...
            observation, img = prepare_observation(
                ...
                corruption_name=cfg.corruption_type
            )
